...
void dyn_array_test(Test* t){
	...
	// Default construction
	{
		...
		t_expect(t, cap(a) == 0, "default cap == 0");
		t_expect(t, raw_data(a) == nullptr, "default data == nullptr");
	}

	// append / len / cap growth
	{
		DynArray<int> a = make_dyn_array<int>(heap_allocator());
	...
	// clear
	{
		...
		t_expect(t, cap(a) == old_cap, "cap unchanged after clear");
		destroy(&a);
	}
	...
	// remove (order-preserving)
	{
		...
		remove(&a, 2); // remove element at index 2 (value 3)
	...
	{
		DynArray<int> a = make_dyn_array<int>(heap_allocator());
		t_expect(t, resize(&a, 32), "resize to 32 ok");
		t_expect(t, cap(a) == 32, "cap == 32 after resize");
		t_expect(t, len(a) == 0, "len unchanged by resize");
		destroy(&a);
	}
